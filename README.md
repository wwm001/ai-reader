...
上書きインストール後は、ChatGPT Reader を開き、`現在のアプリバージョン: 0.1.2-hotfix1 (4)` と新しい `build commit SHA` が表示されることを確認してください。
...
Hotfix1 では、本文に `ChatGPT`、`設定`、`共有`、`コピー`、`モデル`、`アクセス`、`戻る` などの語が含まれていても、長文本文ならUIラベルとして誤除外しないように調整しています。
...
Reader専用ボタンはChatGPT画面で検出後に表示されます。Manifestで対象を `com.openai.chatgpt` に限定しているため、他アプリの画面本文は取得・走査しません。ホームや他アプリへ移動した後にボタンが残る場合でも、本文抽出はChatGPT検出時だけ行います。完全に消したい場合はReaderタイルをOFF、通知の停止、または5秒押しシャットダウンを使ってください。

## Pixel 8 で要確認

次の項目は実機で確認してください。PCやADBなしで、Pixel 8上の操作と診断レポートで判定します。

- 上書きインストール後、`Ver.0.1.2-hotfix1 (4)` と新しいSHAが表示される
...
- ChatGPT → ホーム → 他アプリへ移動しても、他アプリ本文が読み上げ・診断抽出されない
